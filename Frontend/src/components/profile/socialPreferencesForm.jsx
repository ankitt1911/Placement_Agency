...
export default function SocialPreferencesForm({ links, preferences, onLinksChange, onPreferencesChange, errors }) {
  return (
    <>
      ...
      <ProfileSection title="Preferences" icon={MapPin} description="Share your location, salary, and availability preferences for better matching." fields={[{ name: "preferredLocation", label: "Preferred location", placeholder: "Pune, Mumbai, Remote" }, { name: "expectedSalary", label: "Expected salary", placeholder: "600000" }, { name: "currentStatus", label: "Current status", placeholder: "Available", wide: true }]} data={preferences} onChange={onPreferencesChange} errors={errors} />
    </>
  );
}
